...
export default function EditEvent() {
  ...
  useEffect(() => {
    ...
    if (event) {
      ...
      setEnableRegistration(Boolean(event.hasRegistration));
      if (event.registrationFields?.length) {
        setFormFields(event.registrationFields.map((field, index) => ({ ...field, id: String(field.id || index + 1) })));
      }
    }
  }, [id]);
